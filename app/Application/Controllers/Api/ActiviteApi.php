...
use App\Application\Transformers\ActiviteTransformers;
use App\Application\Requests\Website\Activite\ApiAddRequestActivite;
use App\Application\Requests\Website\Activite\ApiUpdateRequestActivite;

class ActiviteApi extends Controller
{

    protected $request;
    protected $model;

    public function __construct(Activite $model , Request $request)
    {
        $this->model = $model;
    ...
    public function index($limit = 10 , $offset = 0 , $lang = "en"){
       ...
       if($data){
             return response(apiReturn(ActiviteTransformers::transform($data))  , 200 );
       }
    ...
    public function add(ApiAddRequestActivite $validation){
    ...
    public function update($id , ApiUpdateRequestActivite $validation){
        $request = $this->checkRequestType();
         $v = Validator::make($this->request->all(), $validation->rules());
         if ($v->fails()) {
            return response(apiReturn('' , 'error' , $v->errors())  , 401 );
