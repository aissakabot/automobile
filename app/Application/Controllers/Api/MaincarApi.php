...
use App\Application\Transformers\MaincarTransformers;
use App\Application\Requests\Website\Maincar\ApiAddRequestMaincar;
use App\Application\Requests\Website\Maincar\ApiUpdateRequestMaincar;

class MaincarApi extends Controller
{

    protected $request;
    protected $model;

    public function __construct(Maincar $model , Request $request)
    {
        $this->model = $model;
    ...
    public function index($limit = 10 , $offset = 0 , $lang = "en"){
       ...
       if($data){
             return response(apiReturn(MaincarTransformers::transform($data))  , 200 );
       }
    ...
    public function add(ApiAddRequestMaincar $validation){
    ...
    public function update($id , ApiUpdateRequestMaincar $validation){
        $request = $this->checkRequestType();
         $v = Validator::make($this->request->all(), $validation->rules());
         if ($v->fails()) {
            return response(apiReturn('' , 'error' , $v->errors())  , 401 );
