...
<div class="container">
        ...
        <form action="{{ concatenateLangToUrl('car/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            ...
            @include("website.car.relation.maincar.edit")
  ...
  <div class="form-group">
   <label for="title">{{ trans("car.price")}}</label>
    <input type="text" name="price" class="form-control" id="price" value="{{ isset($item->price) ? $item->price : old("price")  }}"  placeholder="{{ trans("car.price")}}">
     </div>
  <div class="form-group">
